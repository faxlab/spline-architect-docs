...
- One closed boundary per Streets lot, optional resolved visible lot mesh, and a sampleable graded surface.
...
- **Settings:** Lot Target Area, Max Subdivisions, Cut Mode (Angled/Straight), Irregularity, Gap, Knockout, Boundary Filter (All/Exterior/Interior), Seed, Emit Surfaces, Ground Surface Spacing, Ground Surface Smoothing Iterations.
- Deterministic for a fixed seed. Carries source/material provenance when the input provides it.
...
- **Settings:** Building DataTable row or inline Building Preset; Output; Attach Options for Actors mode.
...
For all three, a valid DataTable row wins; an unresolved configured row warns and falls back to the inline preset. Output modes:

| Output | Generated pin | Dynamic Meshes pin |
| --- | --- | --- |
| **Data** | One point per static piece with transform, bounds, `SA_Mesh`, and source data. | One world-oriented Dynamic Mesh per non-static piece. |
| **Components** | One point carrying `SA_SpawnedActor` for generated actor reference. | Empty. |
| **Actors** | One point carrying `SA_SpawnedActor`; actors remain PCG-managed and respect Attach Options. | Empty. |
